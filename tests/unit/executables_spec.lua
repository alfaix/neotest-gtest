...
describe("executables with single test in file", function()
  ...
  it("executables mapping is persisted via the cache", function()
    setup()
    registry:update_executable(file_id, "/bin/exe1")
    local cache = require("neotest-gtest.cache"):new(registry._cache:path())
    assert.are.same({ [file_id] = "/bin/exe1" }, cache:data())
  end)
end)
