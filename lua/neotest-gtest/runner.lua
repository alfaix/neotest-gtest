    ...
    if #M._runners ~= 0 then
        ...
        if error == nil then
            if choice_idx == nil then
                ...
                choice_idx, runner, error =
                    M.register_runner(selected, nil, {path})
            else
                runner = selected
            end
        end
...
function M.select_runner(index, exe_path)
    if index ~= nil then return M._runners[index] end
    if exe_path ~= nil then
        exe_path = utils.normalize_path(exe_path)
        for _, runner in ipairs(M._runners) do
            if runner:executable() == exe_path then return runner end
        end
    end
    return nil
end
...
function M.load_runners(data, opts)
    ...
    opts = vim.tbl_extend("keep", opts or {},
                          {clear = true, on_error = "notify"})
    local runners = {}
    for _, runner_info in ipairs(data) do
        runners[#runners + 1], err = Runner:new(runner_info.executable_path,
                                                runner_info.compile_command,
                                                runner_info.paths)
        if opts.on_error == "propagate" then
            return false, err
        elseif opts.on_error == "notify" then
            vim.notify(string.format("Error loading executable at %s: %s",
                                     runner_info.executable_path or "<nil>", err),
                       3)
        end
    end
...
function M.register_runner(executable_path, compile_command, paths)
    ...
    if runner == nil then return nil, nil, err end
...
function Runner:set_executable(executable_path)
    ...
    if not exists then
        ...
        return false, string.format("Cannot find an executable at path %s: %s",
                                    executable_path, err)
    end
...
function Runner:set_compile_command(command) self._compile_command = command end
...
local function _is_parent(parent, child)
    ...
    return is_prefix and
               (#parent == #child or child:sub(#parent + 1, #parent + 1) ==
                   Path.path.sep)
end
