...
local function extract_captures(query, source, root)
    ...
    local gettext = function(match, capture_name)
        return vim.treesitter.get_node_text(match[query.captures[capture_name]],
                                            source)
    end
...
local function position_id(position, parents)
    parents = vim.tbl_map(function(pos) return pos.name end, parents)
...
function M.parse_positions_from_string(file_path, query, content)
    ...
    local tree = Tree.from_list(tests_tree, function(pos) return pos.id end)
    return tree
end
