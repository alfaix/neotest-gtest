  ...
  if self._summary == nil then
    ...
    if header_size > 0 then
      ...
      if #full_name >= header_size then
        ...
        local header =
          string.format("%s%s%s%s%s", color, pad_left, full_name, pad_right, COLOR_STOP)
    ...
    elseif status == "passed" then
      lines[#lines + 1] =
        string.format("Passed, Time: %s, Timestamp: %s", test.time, test.timestamp)
    else
      lines[#lines + 1] =
        string.format("Errors: %d, Time: %s, Timestamp: %s", #errors, test.time, test.timestamp)
    end
